...
- **에이전트 팀 순서 규칙**: 사소하지 않은 작업은 라나(외부 리서치)→정찰관(내부 코드, 읽기전용)→Ray 승인→실행관(수정+atomic 커밋, push 안 함)→검증관(사양 대조, 읽기전용) 순. "이미 놓인 다리부터" — 바깥 표준 확인이 안쪽 착수보다 먼저. (현재 라나만 구축됨, 나머지 미착수)
