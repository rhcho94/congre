...
- **CC 보고는 전부 코드 블록 하나에 담는다**: 정찰·조사·감사 보고도, 커밋·push 등 실행
  보고도 백틱 4개(````) 울타리 하나에 통째로 넣는다. Ray가 복사 버튼 한 번으로 코난에게
  넘기기 위함. 울타리를 4개로 하는 이유는 보고 안에 코드 예시(백틱 3개)가 들어가도 박스가
  중간에 끊기지 않게 하기 위함이다. (2026-09-21 Ray 지시)
