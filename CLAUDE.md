...
- **API 에러 키는 서버만 단독 변경 금지**: 서버 에러 응답 본문의 키는 `error`가 표준이나 예외 2곳이 있다(`api/clips/route.ts:61` = `code`, `api/user/delete/route.ts` = `code`+`message`). 두 곳 모두 짝이 되는 프론트가 그 키에 맞춰져 있어 현재 정상 동작한다. 일관성 목적으로 서버 키만 바꾸면 프론트가 못 읽어 범용 에러 문구로 조용히 떨어진다(2026-08-04 `b950821` 버그와 동형). **서버 에러 키를 바꿀 때는 반드시 호출부 프론트를 같은 커밋에서 함께 수정할 것.** 상세는 docs/known-issues.md 참조. (2026-08-05 전수 정찰 확정)
- **감사·진단 작업도 보고 대상**: 문서 책임 규약의 보고 의무는 "코드 변경"만 명시하나,
  코드를 바꾸지 않는 작업(보안 감사, 진단, 정찰)이 오히려 유실 위험이 크다 — 파일
  변경이 0이라 git이 흔적을 남기지 않기 때문. `/security-review` 등 진단성 작업은
  실행 사실과 결과 요약을 코난에게 보고해 docs에 남긴다. (2026-08-06, 과거 감사 실행
  여부를 아무도 확인할 수 없어 세션 초 30분을 소모한 데서)
...
- **증상을 볼 때 "정상 사례가 있는가"를 먼저 묻는다**: 2026-09-20 세로 영상이 눕는 문제를 한 이벤트만 보고 "Shotstack이 회전 메타데이터를 원래 무시한다"로 일반화했다. 같은 날 정상으로 나온 다른 이벤트 하나를 대조하자 가설이 즉시 깨졌다. 전칭 판단 전에 반례를 찾는다.
...
- **랜딩 배포 후 실측 전에 CLI 출력의 `Aliased`·`Ready` 줄부터 확인**: 2026-09-16 로그인이 풀린 채 배포가 에러로 멈췄는데 실측부터 해서, 예전 배포본을 보고 제외 규칙이 뚫렸다고 오판할 뻔했다. 배포 출력 확인이 실측의 선행 게이트다.
...
- **파일명이 대상 범위를 말한다**: 부분 일치로 대체 문서를 읽지 않는다.
- **known-issues 등재 범위를 그대로 작업 범위로 쓰지 않는다**: 정찰로 재확인한다.
- **리다이렉트로 진입하는 페이지는 `currentUser`를 바로 믿지 않는다**: 인증 복원 대기가 필요하다.
- **문서 형식이 안 맞으면 배치를 의심한다**: 서식이 어긋나면 그 문서가 그 자리에 있을 것이 아닐 수 있다.
- **선택지 라벨은 한 메시지에 한 세트만 쓴다**: 두 세트가 있으면 사용자가 다른 세트의 라벨로 답한다.
- **부등호 방향을 확인하지 않고 배치 주기를 위험으로 읽지 말 것**: 2026-08-13 핸드오프가
  cleanup을 "1일 1회라 25시간에 삭제될 수 있다"고 적었으나, 비교식이
  `participantNotifiedAt <= now - cutoff`라 배치는 삭제를 늦추는 방향으로만 작동한다.
  주기가 길다는 사실만으로 위험을 추론하기 전에 비교식을 읽는다. 실제 보관 구간은
  [컷오프, 컷오프+주기].
- **L 번호 최댓값을 grep으로 뽑을 때 코드 줄 참조가 섞인다**: known-issues에는 이슈 번호
  L4~L14와 코드 줄 참조(`layout.tsx L72`, `index.html L3627`)가 같은 파일에 공존한다.
  `L[0-9]+` 패턴은 둘을 구분하지 못해 최댓값이 L3636처럼 나온다. 이슈 번호는 `### L숫자.`
  형태의 소제목만 센다. (2026-08-14 실측: 최댓값 L14, 빈 번호 L1~L3·L6)
- **"메일 안 옴" 사고 = send vs deliver 먼저 가름**: 발송 자체 실패(send)와 도달 실패(deliver)는 다른 층. mail-tester(중립 수신서버) 미도착 + 브라우저 콘솔 sendOobCode 400이면 send-side(도메인 미승인 등). Vercel 서버로그에 안 남을 수 있음(브라우저→Firebase 직접 호출). "스팸 간다"는 known-issue에 끌려 도달성부터 의심 말 것.
