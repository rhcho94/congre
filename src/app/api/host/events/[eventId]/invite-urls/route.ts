...
export async function GET(
  ...
  try {
    ...
    if (!isS3Configured) {
      return Response.json({ introMediaUrl: null, outroMediaUrl: null });
    }
    ...
    const [introMediaUrl, outroMediaUrl] = await Promise.all([
    ...
    return Response.json({ introMediaUrl, outroMediaUrl });
  } catch (err) {
