...
## 2026-09-16 (22) — 배포 제외 규칙을 차단 목록 방식으로 신설

- 상태: `deploy/.vercelignore` 신설. 뺄 파일을 패턴으로 적는 차단 목록 방식. `uploads/`는 현행 페이지가 쓰는 jpg 5개를 남기려고 확장자(mp4·png·jpeg·md)로 제외했다.
- 이유: 허용 목록(올릴 것만 적는 방식)과 비교해, 새 이미지를 추가할 때 규칙 수정이 필요 없는 쪽을 택했다(Ray 결정).
- 함의: 규칙에 없는 새 형식의 파일(예: `.txt` 메모, uploads의 새 확장자)이 생기면 공개된다. 백업은 `*_backup.html` 이름 관례를 지키면 자동 제외된다. CD zip 덮어쓰기 시 `.vercelignore` 존재를 확인한다(known-issues L8 ⑤).

## 2026-08-19 — FAQ·소개 페이지 신설 결정 3건

**결정 1: FAQ를 index.html 내부 섹션이 아니라 faq.html 별도 파일로 만든다**
- 이유: index.html은 CD zip으로 통째 덮어쓰이는 파일이라, 직접 수정분이 하나 늘 때마다 다음 CD 작업 시 확인 항목이 늘어난다(현재 L8에 4건 등재). 별도 파일이면 그 목록이 늘지 않는다. pricing.html 선례가 있고 cleanUrls도 이미 켜져 있다.

**결정 2: 회사 "소개"에 사업자 정보를 넣어 about.html로 신설한다**
- 상태: 사업자 정보 7항목의 사본이 3곳(앱 terms·앱 privacy·랜딩 푸터)에서 4곳으로 늘었다.
- 이유: 카드사 심사관과 학교·기업 담당자 모두 회사 실체를 확인한다. 소개 페이지의 값이 사본 1개 관리 비용보다 크다고 판단했다.
- 함의: 통신판매업 신고번호 발급 시 / 070 번호 신청 시 고쳐야 할 곳이 4곳이다. known-issue "사업자 정보 블록이 두 페이지에 독립 하드코딩" 항목의 범위가 늘었다.
- 이메일 배분 유지: about.html의 문의 블록은 cs@rayne.co.kr, 사업자 정보 블록은 ray@rayne.co.kr이다. 2026-08-11 결정 2(법정 표시는 실명 계정, 고객 창구는 역할 계정)를 그대로 따른다. 임의 통일 금지.

**결정 3: 랜딩 톤 이원화(메인 라이트 / 하위 페이지 다크)를 현 상태로 둔다**
- 상태: index.html은 크림·연두 라이트, pricing.html은 다크다. 신설 faq.html·about.html은 pricing 계열을 따라 다크다.
- 이유: 톤 불일치는 이번 작업이 만든 것이 아니라 index↔pricing에 원래 있던 것이다. 통일하려면 CD 사이클이 필요하고, 카드사 심사 마감(8/24)을 앞둔 시점에 미관을 위해 기능 공백("준비 중" 토스트)을 유지하는 것은 교환비가 맞지 않는다.
- 격상 트리거: 다음 랜딩 디자인 사이클.

## 2026-08-11 (21) — 랜딩 푸터 통신판매업신고번호 값 교체

- 상태: 랜딩 `index.html:3768` 통신판매업신고번호 값 교체, 배포 `dpl_5qTqWwk7iwQsBjCkhV4Z7QEb3eFW`, congre.kr·www.congre.kr 양쪽 실물 확인
- 이유: 앱 terms·privacy와 사업자 정보 7항목이 문자 단위로 일치해야 한다는 요건(2026-08-11 결정 3) 때문에 앱만 바꾸면 불일치가 남는다. 트랙이 갈리므로 같은 사이클에서 처리했다
- 함의: 앱은 커밋 `bc0a92f`, 랜딩은 git 외부라 배포로만 존재한다. 두 트랙은 서로의 작업으로 해소되지 않는다
