...
## 2026-09-21 — 카카오 4019 원인: 커스텀 도메인 이전 시 콘솔 등록 누락

- **현상**: 카카오톡 공유 버튼이 4019 에러로 실패했다.
- **원인**: 커스텀 도메인(`app.congre.kr`)으로 이전하면서 카카오 개발자 콘솔에 새 도메인을
  등록하지 않았다. 코드 문제가 아니었다.
- **처리**: 콘솔 두 곳에 `https://app.congre.kr`를 추가해 해결. 코드 변경 없음.
  - [제품 설정 > 제품 링크 관리] 웹 도메인
  - [앱 설정 > 플랫폼 키 > JavaScript 키] SDK 도메인
- **교훈**: 도메인을 옮길 때 외부 SaaS 콘솔의 도메인 등록을 같이 점검한다. 카카오는 등록처가
  두 곳으로 나뉘어 있어 한 곳만 넣으면 증상이 남는다.
