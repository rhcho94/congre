...
## 2026-09-21 — 카카오 공유 카드 이미지는 공용 `og-image.png`로 통일

- **결정**: `Kakao.Share.sendDefault`의 `content.imageUrl`을 `${appUrl}/og-image.png`로
  맞춘다. 완성 페이지(`share/[eventId]/page.tsx:50`)와 대시보드
  (`dashboard/events/[eventId]/page.tsx:582`) 두 입구 모두. 커밋 `4bb80a7`.
- **문제**: 같은 완성 영상을 공유하는데 링크 붙여넣기(OG)와 카카오톡 버튼의 카드 그림이
  달랐다. OG는 `og-image.png`(1200×630), 카카오 버튼은 `logo.png`(1080×630, 회색 배경 로고).
- **두 경로는 별개다**: `sendDefault`는 JS가 `content.imageUrl`을 호출 시점에 직접 넘기고,
  링크 붙여넣기는 카카오 스크랩 봇이 대상 페이지의 OG 메타태그를 읽는다. 따라서 OG 태그만
  고쳐서는 카카오 버튼 카드가 바뀌지 않는다.
- **기각 — 안 A (`/api/og-image/<eventId>` 사용)**: OG와 완전히 같은 URL을 쓰면 인트로
  이미지를 올린 이벤트는 카드에도 그 사진이 나온다. 그러나 인트로 이미지가 없으면 이
  라우트가 302 리다이렉트를 반환하는데, 카카오 스크래퍼가 리다이렉트를 따라가는지 확인하지
  못했다. 인트로 이미지 원본은 규격·비율도 제각각이라 카드 표시가 보장되지 않는다.
  2단계로 이월. known-issues 참조.
- **대시보드를 함께 바꾼 이유**: 두 버튼은 같은 것을 공유하는 두 입구다(같은 `shareUrl`,
  같은 `description`). 한쪽만 고치면 "대시보드에서 보낸 카드와 완성 페이지에서 보낸 카드가
  다르다"는 같은 종류의 불일치가 새로 생긴다.
- **함께 정리**: `logoUrl` 변수·prop 이름을 내용에 맞는 `shareImageUrl`로 바꿨다
  (`share/[eventId]/page.tsx:50,73`, `ShareActions.tsx:14,17,50`).
- **미확인**: 카카오 공식 문서(`developers.kakao.com`)가 조사 환경에서 접근 차단돼
  feed 이미지의 권장 비율·최소 크기·최대 용량·캐싱 동작을 원문으로 확인하지 못했다.
  검색 스니펫 수준의 수치(권장 800×400 / 2:1 / 10MB 이하)는 코드나 문서에 박지 않았다.
