...
## 2026-05-28 — emailChannel reply-to 지원 (어댑터 확장)

- **결정**: `emailChannel.send()` 어댑터에 `replyTo?` 옵션 필드 추가 (`types.ts` + `email.ts`). 리드 폼 회신을 위해 신청자 이메일을 reply-to로 설정.
- **이유**: 사양에 "reply-to 신청자 이메일"을 박았으나 기존 어댑터에 전달 경로 없었음. 우회 대신 옵션 A(어댑터 확장) 채택 — 추상화 유지, 기존 7개 호출처 무영향.
