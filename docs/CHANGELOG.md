...
- feat(payment): 유료 플랜도 결제 전에 마감이 확정되도록 변경. 마감 확인 화면에
  업로드 수·포함 수를 표시하고, 포함 클립이 0개면 마감·결제를 진행할 수 없다.
  결제 전 마감을 되돌리는 경로(`/api/events/[eventId]/reopen`)를 신설해
  업로드 토큰을 그대로 복구한다.

## 2026-09-16

- fix(legal): 약관·개인정보처리방침의 통신판매업신고번호를 신고번호로 교체 —
  2026-09-10 신고 완료(제2026-성남분당B-0854호)에 따라 `terms/page.tsx`·
