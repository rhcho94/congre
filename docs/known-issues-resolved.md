...
## ✅ 클립 0개로 결제하면 렌더가 실패한다 (2026-09-21 해소)

- **해소: 2026-09-21** — `987e5d5`로 `payment/prepare`에 클립 0개 가드를 원복했다
  (`src/app/api/payment/prepare/route.ts:56-58`, `clipCount === 0`이면 400 `NO_CLIPS`).
  `f58281d`가 제거한 것과 같은 위치·같은 형태이며, mode 판정 뒤에 있어 first·rerender
  두 경로 모두에 적용된다. 같은 커밋에서 도달 불가가 된 클립 0개 경고 배너를 삭제했다.
- **재현 사례 (2026-09-21 10:49 KST)**: 테스트 결제에서 결제 승인이 끝난 뒤
  `render/start`가 400 `NO_CLIPS`로 거절했다. 근거는 Vercel 로그의 Path
  (`/api/render/start`)와 실화면("결제는 완료됐습니다 / 다만 영상 생성 시작에는
  실패했습니다"). 즉 등재 당시 기술한 "결제는 되는데 렌더는 실패"가 실제로 재현됐다.
- **실화면 검증 (2026-09-21)**: 클립 0개 유료 이벤트에서 [마감하기]를 누르면 결제 페이지가
  "업로드된 영상이 없어 결제할 수 없습니다."만 표시하고 결제 위젯을 띄우지 않는다.
  문구는 `payment/[eventId]/page.tsx:28`의 `prepareErrorMessages.NO_CLIPS`에서 온다.
- **관련 결정**: `decisions/market-product.md` 2026-08-24 "클립 0개 결제 허용"은 같은 날
  폐기 표시했다. 토스 심사 트랙 종료로 가드를 뺀 사유가 사라졌다.
...
## ✅ 클립 0개 시 경고 배너와 제작 기간 고지가 상반돼 보인다 (2026-09-21 해소)

- **해소: 2026-09-21** — `987e5d5`가 `payment/[eventId]/page.tsx`의 클립 0개 경고 배너
  블록을 삭제해 발현 조건 자체가 사라졌다. 같은 커밋의 `prepare` 가드 때문에 클립 0개
  이벤트는 결제 페이지에서 `prepareErrorMessages.NO_CLIPS` 화면으로 떨어지고, 배너가
  있던 결제 요약 화면에 도달하지 않는다. 문구를 손대서 푼 것이 아니라 화면이 없어진
  형태의 해소다.
...
## ✅ CLIP_COUNT_CHANGED 안내가 재발행 경로를 못 짚는다 (2026-09-21 해소)

- **해소: 2026-09-21** — `9dd3a04`로 등재 당시 적어 둔 "해소 방법" 두 가지를 모두
  반영했다. (1) 409 응답 본문에 `eventId`를 추가했다
  (`src/app/api/payment/confirm/route.ts:73`, 값은 `:45`의 `order.eventId`). (2) 409 화면에
  주 버튼 [새 금액으로 결제하기] → `/payment/{eventId}`를 넣고 보조로 기존 대시보드
  링크를 남겼다(`src/app/payment/success/page.tsx:190-201`). 안내 문구는
  "아래 버튼으로 새 금액을 확인하고 다시 결제해 주세요."로 바꿨다.
  `eventId`가 없을 때를 대비한 폴백은 대시보드 링크만 띄우고 문구도
  "대시보드에서 이벤트를 열어 다시 결제를 진행해 주세요."로 구분한다.
- **안전성 확인**: 409는 인증(`:10-11`)과 주문 소유 확인(`:33-35`)을 통과한 뒤에만
  도달한다. `order.hostId`는 `prepare`가 이벤트 주최자 확인 후 기록한 값이므로
  (`prepare/route.ts:32-34`, `:89`), 응답에 `eventId`를 실어도 남의 이벤트 ID가 새지 않는다.
- **재결제가 성립하는 이유**: 409 시점에 `confirm`은 events 문서를 건드리지 않는다
  (이벤트 쓰기는 토스 승인 성공 이후인 `:135-147`). 이벤트가 `status: "open"`으로 남아
  `prepare`가 다시 first mode로 판정하고(`prepare/route.ts:41-42`), 재집계한 클립 수로
  새 금액과 새 `orderId`를 만든다(`:67`, `:78`).
- **실화면 종단 검증 (2026-09-21, 테스트 키)**: 결제 페이지를 연 뒤 다른 기기에서 영상을
  추가 → 결제 → 409 화면 → [새 금액으로 결제하기] → 새 개수 기준 결제 페이지 →
  결제 승인 → 렌더 시작 → 편집 완료까지 확인했다.
...
- **해소: 2026-09-01** — 토스 제작 기간 기재 작업과 함께 랜딩 각주를 3항목으로
  재구성하며 "참가자 영상 보관 기간(48시간)이 지나면 재편집할 수 없습니다."를
  기재했다. `deploy/pricing.html` 수동 배포.
...
## ✅ render_delayed 계열 "30분" 문구가 실동작과 불일치 (2026-08-17 해소)

- **원 발견**: 2026-08-14 scout 정찰(환불 구간 실측, 요청 범위 밖 발췌분). 지연 알림
  3곳이 "30분"을 하드코딩하고 있었다.
...
- **해소**: 2026-08-17 `bf93b72`. 동적 치환이 아니라 기산점을 문구에 명시하는 방식을
  택했다 (`paidAt` + 4시간). 상대시간 표현을 버렸으므로 클립 수와 무관하게 항상
  정확해졌다.
- **등재 오차 정정**: 원 등재는 문구 3곳이었으나 실제로는 4곳이었다.
  `cron/check-render-deadlines/route.ts:69` 주석("T+E+30분")이 등재에서 빠져 있었고
  같은 커밋에서 함께 정정했다(코드 로직 무변경, 주석만).
- **잔존**: SMS 템플릿이 90바이트 제약을 넘는다 — known-issues에 별도 등재.

## ✅ 회원가입 화면 "무료로 테스트해 볼 수 있어요" 문구 — 카드사 심사 맥락 충돌 (2026-08-15 해소)

- **원 발견**: `/signup` 상단 안내가 "가입하면 바로 무료로 테스트해 볼 수 있어요"였다. 실제
  의미는 무료 체험 안내이나 토스 1차 심사 지적이 "테스트 상품 불가"였으므로, 제출 캡처
  ④(로그인·회원가입)에 이 단어가 찍히면 오해 소지가 있었다.
- **해소**: `c29efdc`로 문구에서 "테스트"를 제거했다.
- **잔존**: 없음.
...
- **등재 없이 해소된 항목**: 2026-08-13 핸드오프가 "known-issues 신규 등재 5건" 중 하나로
  적었으나 실제로 `known-issues.md`에 등재된 적은 없다. 시간축을 정확히 남기기 위해 해소
  이력만 기록한다.
- **원 상태**: 2026-08-13 `f0edbb4`로 약관 제11조 ⑤가 "회사의 사유로 제작이 지연·실패한
  이벤트의 클립은 최대 7일까지 보존되며 위 기간이 경과하면 예외 없이 삭제된다"고 게시됐으나,
  `cron/cleanup`의 순회 쿼리가 `where("status","==","done")` 하나뿐이라 지연·실패 이벤트가
  순회 대상에 들어오지 않았다. 즉 약관 게시 시점부터 하루 동안 선언과 이행이 어긋난
  상태였다.
- **해소**: 2026-08-14 `4cdcea2`(`closedAt` 신설) + `3adec53`(D-3 루프 신설).
  `status in ["closed","rendering"]`을 훑어 `closedAt` 기준 7일 경과 클립을 삭제한다.
  실동작 확인 — cleanup 수동 실행에서 `closedAt` 없는 문서 3건 warn + HTTP 200.
- **관련 결정**: `decisions/data-flow.md` 2026-08-14 `closedAt` 섹션.

## ✅ 약관이 "무료 베타" 전제 — 유료 도입과 조문 모순 (2026-08-14 해소)

- **원 발견**: 2026-08-10 세션, 등재는 2026-08-11. 제10·14·20조가 유료 도입과 모순되고
  환불 조항이 부재하며 제10조 ③이 "별도 고지"로 미뤄져 있었다. 2026-08-11 세션에서 조문
  개정안 확정 초안(`docs/legal/terms-privacy-v1.0-draft.md`)까지 만들었으나 코드 미적용
  상태였다.
- **해소**: 2026-08-13 `f0edbb4` 약관 v1.0 적용(제10조 개편, 제10조의2 결제·제10조의3
  청약철회 및 환불 신설, 제11조 ⑤·제14조·제20조 정정), 2026-08-14 `e770a5d` 개인정보
  처리방침 v1.0 적용(제2조 결제 정보 수집 항목·③항, 제3조 법정 보존기간 3행·④ 1호·⑤,
  제5조 토스페이먼츠 위탁).
- **개정 범위 정정**: 초안이 "처리방침 4곳"으로 적었으나 실제는 5곳(⑨~⑬)이었다.
- **잔존**: 약관 제10조의3 ③의 환불 구간이 코드와 불일치한다 — known-issues에 별도
  등재(★ 토스 PPT ③장 캡처 전 해소 필요). 조문 게시 자체는 완료이므로 본 항목은 해소로
  처리한다.

## ✅ 유료 플랜 UI 잠김 — 토스 심사 "테스트 상품 불가" 요건과 충돌 (2026-08-13 해소)

- **원 발견**: 2026-08-10 세션, 등재는 2026-08-11. `comingSoon: true` + "준비 중" 라벨로
  유료 플랜 UI가 잠겨 있어 토스 심사 요건 "테스트 상품 불가"와 충돌했다.
- **해소**: 2026-08-13 `16e3afc`(코드 ②)에서 `comingSoon` 해제. 2026-08-13 세션 결정 6.
- **주의**: 결제 요약 화면 캡처 시 쿠폰을 입력하면 안 된다 — `dashboard/create/page.tsx:310`이
  쿠폰 입력 시 플랜 라디오를 숨긴다.
