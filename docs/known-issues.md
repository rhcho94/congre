...
## 클립 0개로 결제하면 렌더가 실패한다 (2026-08-24)

`payment/prepare`의 클립 0개 가드를 제거(`f58281d`)하면서
"결제는 되는데 렌더는 실패"하는 경로가 열렸다. `render/start`에는
NO_CLIPS 가드가 그대로 남아 있어 클립이 없으면 렌더를 거부한다.
2026-08-24 실측에서 결제 완료 후 "영상 생성 시작에는 실패했습니다"
화면 확인.

결제 페이지 경고 배너로 고지하나 코드상 차단은 없다. 약관상 결제
후 4시간 이내는 환불이 제한되므로 분쟁 소지가 있다.

실사용자 0명 + 테스트 키 상태라 현재 실피해는 없다. 유료 개시 전
필수 처리 항목(기존 2건에 이어 3번째).
...
## 클립 0개 시 경고 배너와 제작 기간 고지가 상반돼 보인다

- **현황**: 결제 페이지에서 클립이 0개일 때만 뜨는 경고 배너
  (`payment/[eventId]/page.tsx`, `prepared.clipCount === 0` 조건)가
  "결제 후 4시간 이내에는 환불이 제한됩니다"라고 적는다. 2026-09-01에 추가한
  제작 기간 고지는 "4시간을 초과하면 결제 금액의 50%가 환불 확정"이라 적는다.
  같은 화면에 4시간이 두 번, 서로 반대 방향으로 나온다.
- **실제로는 상반되지 않는다**: 배너는 청약철회 제한 축(약관 제10조의3 ①),
  고지는 회사 귀책 지연 축(같은 조 ③)이다. 둘 다 참이다.
- **영향**: 클립 0개 상태에서만 발현한다. 정상 결제 흐름에서는 배너가 뜨지 않는다.
  실피해 없음. 심사 제출 캡처는 클립 2개 상태로 떠서 회피했다.
- **처리**: 등재만. 문구 정리는 두 축의 표현을 함께 손봐야 하므로 범위가 있다.
- **격상 트리거**: 실고객 문의 발생 시 / 결제 페이지 문구를 다시 손대게 될 때.
- **출처**: 2026-09-01 세션.
