...
## 랜딩 pricing에 재편집 기한(48시간)이 빠져 있다

- **현황**: 랜딩 `pricing.html`이 "재편집 시 처음 금액의 80%로 재결제"만 안내하고 기한을 적지 않는다. 실제 재편집 가능 창은 완성본 출력 후 48시간이다(그 뒤 참가자 클립이 삭제돼 재렌더 경로 자체가 없다).
- **영향**: 고객이 가격 조건의 일부만 보고 판단한다. 결제 전 고지 관점에서 누락으로 읽힐 여지가 있다.
- **처리**: 랜딩은 git 외부 별도 배포 폴더라 본 저장소 작업 범위 밖이다. 문구 추가 후 Vercel CLI 배포가 필요하다.
