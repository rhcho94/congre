...
## 워터마크 시각 사양 정밀 수치 — CD 시안 검토 영역

- **현황**: 2026-05-29 (3) 결정에서 워터마크 시각 사양 박힘. 위치·텍스트·색·폰트는 확정. font.size "중간" 정밀치 + font.opacity "은은하게" 정밀치 + 우하 offset 정밀치 미정.
- **결정 영역**: 운영자 + CD. 워터마크 박은 시안 / 안 박은 시안 비교 검토 후 결정 가능.
- **잠재 수치**: font.opacity 0.3~0.5 영역, font.size 32~48 영역, offset {x: -0.04, y: -0.04} 영역 (Shotstack 공식 워터마크 예시 기준).
- **격상 트리거**: 워터마크 구현 트랙 진입 직전.
- **관련 결정**: decisions/market-product.md 2026-05-29 (3) 5번.
