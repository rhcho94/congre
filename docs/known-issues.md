...
## 결제 성공 후 뒤로가기를 누르면 "이미 처리된 결제입니다" 화면이 뜬다

- **현황**: `src/app/payment/success/page.tsx:130`이 결제 성공 후 이벤트 페이지로 갈 때
  `router.push`를 쓴다(`replace`가 아니다). success 페이지가 히스토리에 남고, 뒤로가기로
  다시 열리면 `confirm`이 재호출된다. 주문은 이미 `paid`이므로
  `api/payment/confirm/route.ts:37-39`가 400 `ORDER_NOT_PENDING`을 주고,
  success 페이지의 `:96-99` 분기가 "이미 처리된 결제입니다" 화면을 띄운다.
  중복 호출 방어인 `called.current`(`:26`, `:35-36`)는 같은 마운트 안에서만 유효해
  페이지를 다시 열면 초기화된다.
- **안전성**: 결제가 중복되지는 않는다. `ORDER_NOT_PENDING` 가드가 토스 승인 호출
  (`:90`)보다 앞에 있어 두 번째 승인 요청 자체가 나가지 않는다. 고객이 낯선 화면을
  보는 문제다.
- **실화면 확인**: 2026-09-21 발생 확인.
- **보류 이유**: `replace`로 바꾸면 뒤로가기 목적지가 결제 페이지나 토스 쪽 페이지로
  바뀐다. 결제 페이지로 가면 이벤트가 이미 `closed` + `unlocked: true`라
  `prepare`가 400 `INVALID_EVENT_STATE`로 거절할 것으로 **추정된다(미검증)**.
  즉 낯선 화면이 다른 낯선 화면으로 바뀔 뿐이라, 어디로 보낼지 설계한 뒤 착수한다.
- **처리**: 등재만.
- **격상 트리거**: 고객 문의 발생 시.
- **출처**: 2026-09-21 세션.

## 결제 요청 오류 팝업이 토스의 거절 사유를 가린다

- **현황**: `src/app/payment/[eventId]/page.tsx:142-144`의 `handlePay` catch가
  `alert("결제 요청 중 오류가 발생했습니다.")`라는 일반 문구를 띄운다. 토스 위젯이
  던진 사유(`err`)는 화면에 나오지 않는다.
- **로그는 있다**: 같은 catch의 `:142`에 `console.error("[payment] requestPayment failed:", err)`가
  이미 있어 브라우저 콘솔에는 남는다. CLAUDE.md:155 규칙은 충족한다.
- **발생 사례**: 2026-09-21 카드사를 선택하지 않은 상태에서 결제 버튼을 눌러 발생.
- **영향 낮음**: 토스 위젯이 "카드사를 선택해주세요"를 인라인으로 표시하므로, 고객은
  alert를 닫으면 위젯의 실제 안내를 볼 수 있다.
- **처리**: 등재만.
- **격상 트리거**: 결제 요청 실패 관련 고객 문의 발생 시 / 결제 페이지를 다시 손댈 때.
- **출처**: 2026-09-21 세션.

## 거절 응답 중 서버 로그가 남지 않는 곳 6군데

- **현황**: 아래 6곳이 4xx를 반환하면서 `console.error`를 남기지 않는다. Vercel 로그에
  흔적이 없어 사후 진단이 안 된다.
  - `src/app/api/payment/confirm/route.ts:20` — 400 `INVALID_REQUEST`
  - `src/app/api/payment/confirm/route.ts:38` — 400 `ORDER_NOT_PENDING`
  - `src/app/api/payment/confirm/route.ts:42` — 400 `AMOUNT_MISMATCH`
  - `src/app/api/payment/confirm/route.ts:56` — 400 `INVALID_EVENT_DATA`
  - `src/app/api/render/start/route.ts:65` — 400 `NO_CLIPS`
  - `src/app/api/render/start/route.ts:77` — 400 `NO_CLIPS_AFTER_EXCLUSION`
- **규칙과의 관계**: CLAUDE.md:155는 "catch 블록"만 명시한다. 위 6곳은 catch가 아니라
  조건문 분기라 규칙의 글자에서 빠져 있다.
- **이미 메운 곳**: `e523b29`로 세 곳을 채웠다 — `confirm`의 토스 거절 분기
  (`confirm/route.ts:110`), `render/start`의 `createRender` catch
  (`render/start/route.ts:198`), 대시보드 재렌더 버튼 catch
  (`dashboard/events/[eventId]/page.tsx:812`).
- **우선순위 메모**: `render/start:65`의 `NO_CLIPS`는 2026-09-21 10:49 사고의 실제 원인이었고
  로그가 없어 원인 규명에 시간이 걸렸다. 다만 `987e5d5`로 `prepare` 단계에서 막히게 돼
  결제 후 이 분기에 도달하는 경로는 닫혔다.
- **처리**: 등재만.
- **격상 트리거**: 원인 미상의 4xx 문의 발생 시 / 해당 라우트를 다시 손댈 때.
- **출처**: 2026-09-21 세션.

## prepareErrorMessages에 죽은 키가 2개 있다

- **현황**: `src/app/payment/[eventId]/page.tsx:26-27`의 `ALREADY_UNLOCKED`
  ("이미 결제가 완료된 이벤트입니다.")와 `EVENT_NOT_OPEN`("이미 마감된 이벤트입니다.")은
  현재 `prepare`가 반환하지 않는 코드다. `prepare`가 주는 에러 코드는
  `UNAUTHORIZED`·`EVENT_ID_REQUIRED`·`EVENT_NOT_FOUND`·`FORBIDDEN`·`NOT_PAID_PLAN`·
  `INVALID_EVENT_STATE`·`NO_CLIPS`·`INVALID_EVENT_DATA`·`MISSING_FIRST_AMOUNT`·
  `ORDER_SAVE_FAILED`다.
- **추정**: `8290dbe`(2026-08-16 mode 자동 판정) 이후 두 상황이 `INVALID_EVENT_STATE`
  하나로 통합된 것으로 보인다. 커밋 diff로 확인하지는 않았다.
- **`NO_CLIPS`는 살아 있다**: `987e5d5`로 `prepare`에 가드가 돌아와 다시 쓰인다.
- **영향**: 없음. 맵에 안 걸리면 `:82-84`의 기본 문구로 떨어진다.
- **처리**: 등재만.
- **격상 트리거**: 결제 에러 문구를 정리할 때 함께.
- **출처**: 2026-09-21 세션.

## 환불 집행 코드가 없다 — 수동 처리 추정

- **현황**: `src/app/api/`에 토스 결제취소 API(`/v1/payments/{paymentKey}/cancel`) 호출이
  0건이다. `check-render-deadlines`는 `refundStatus`("50"/"100")와 알림 시각만 기록하고,
  환불 메일은 카카오톡 @congre로 절차 문의를 안내한다.
- **미확인**: 실제 환불을 토스 대시보드에서 수동 집행하는지. Ray 확인 필요.
- **영향**: `refundStatus`는 "환불 대상 확정"이지 "환불 완료"가 아니다. 라이브 키 전환 후
  지연 환불이 발생하면 운영자가 직접 취소해야 한다.
- **처리**: 등재만.
- **격상 트리거**: 라이브 키 전환 직전 / 첫 실환불 발생 시.
- **출처**: 2026-09-19 세션.

## S3 도착 실패로 rendering에 고착된 이벤트를 빼내는 코드가 없다

- **현황**: `check-rendering`은 S3 HeadObject 실패 시 건너뛰고 재시도 카운터가 없어 5분마다
  무한 재시도한다. `check-render-deadlines`(`vercel.json:3`)는 알림·환불 표시만 하고
  `status`를 바꾸지 않는다.
- **영향**: Shotstack이 약관상 24시간 뒤 렌더 자산을 지우면 영상은 영구 손실되고 이벤트는
  `rendering`에 남는다. 주최자는 재시작 버튼도 받지 못한다.
- **처리**: 등재만.
- **격상 트리거**: 라이브 키 전환 전 검토 권장 / 실제 고착 사례 관측 시.
- **출처**: 2026-09-18 세션.

## D-3 정체 정리가 렌더 중인 이벤트의 클립도 지운다

- **현황**: `cron/cleanup/route.ts:118-141`이 `status in ["closed", "rendering"]` 중
  `closedAt`이 7일 이전이고 `clipsDeletedAt`이 없으면 클립을 지운다. 렌더 진행 여부를
  보지 않는다.
- **영향**: 7일 전에 마감된 이벤트가 재렌더를 시작해 `rendering`이 되면 다음 cleanup(1일
  1회)에 클립이 삭제될 수 있다. Shotstack이 그 전에 소스를 받아갔는지는 코드로 판정 불가.
- **추정**: done 이벤트의 클립은 참가자 알림 후 48시간에 지워지므로 실제로 겹치는 경우는
  드물다.
- **처리**: 등재만.
- **격상 트리거**: cleanup 또는 재렌더 흐름을 다시 손댈 때.
- **출처**: 2026-09-19 세션.

## Shotstack 자산 삭제 실패가 조용하다

- **현황**: `src/lib/shotstack.ts:379-404` 삭제 함수의 실패 경로 3개가 조용하다. 목록 조회
  실패와 빈 목록은 로그가 없고, DELETE는 `res.ok`를 확인하지 않는다.
- **확인**: 2026-09-18 Shotstack 대시보드 대조로 현재는 정상 삭제됨(`Deleted` 배지).
- **영향**: 실패해도 알 방법이 없다. 개인정보 파기와 연결된 경로다.
- **처리**: 등재만.
- **격상 트리거**: 삭제 경로 수정 시 / 정기 점검 시.
- **출처**: 2026-09-18 세션.

## render/start가 presigned URL 일부를 로그에 남긴다

- **현황**: `render/start/route.ts`(2026-09-18 기준 :106)가 presigned URL 앞 80자를 콘솔에
  출력한다. `shotstack.ts:346-347` 주석은 서명 URL을 절대 기록하지 않는다고 명시해 서로
  어긋난다.
- **미확인**: 앞 80자에 서명 값이 포함되는지.
- **처리**: 등재만.
- **격상 트리거**: render/start 수정 시.
- **출처**: 2026-09-18 세션.

## 쿠폰 이벤트는 완성 후 재렌더를 할 수 없다

- **현황**: 쿠폰 이벤트는 `plan: "paid"`라 done 화면의 재렌더 모달이 결제 페이지로 보내고,
  `firstPaidAmount`가 없어 `prepare`가 400 `MISSING_FIRST_AMOUNT`("이 이벤트는 재편집
  결제를 지원하지 않아요")를 반환한다. 모달은 결제한 적 없는 쿠폰 이벤트에도 "처음 결제
  금액의 80%" 문구를 띄운다.
- **처리**: 등재만. 의도된 제약인지는 결정한 적 없다.
- **격상 트리거**: 쿠폰 사용자의 재렌더 문의 시.
- **출처**: 2026-09-19 세션.

## 결제 페이지 에러 문구 테이블이 prepare 응답 코드와 어긋난다

- **현황**: `payment/[eventId]/page.tsx:24-33` 테이블에 `prepare`가 반환하는
  `UNAUTHORIZED`·`EVENT_ID_REQUIRED`·`INVALID_EVENT_DATA`·`ORDER_SAVE_FAILED`가 없고,
  `prepare`가 반환하지 않는 `ALREADY_UNLOCKED`·`EVENT_NOT_OPEN`·`NO_CLIPS`가 있다.
- **미확인**: 테이블에 없는 코드가 왔을 때의 폴백 문구.
- **처리**: 등재만.
- **격상 트리거**: 결제 페이지 수정 시.
- **출처**: 2026-09-19 세션.

## 재렌더 기한 문구와 실제 클립 삭제 기산점이 다르다

- **현황**: done 화면 문구는 "완성본이 나온 뒤 48시간"이나, 실제 삭제는
  `notifications.participantNotifiedAt` + 48시간이다(`cron/cleanup/route.ts:43-50`).
  `participantNotifiedAt`이 찍히지 않으면 클립이 삭제되지 않는다.
- **미확인**: `participantNotifiedAt`이 찍히지 않는 사례가 실데이터에 있는지.
- **처리**: 등재만.
- **격상 트리거**: 재렌더 기한 문의 시 / cleanup 수정 시.
- **출처**: 2026-09-19 세션.

## 환불 잠금 안내가 대시보드 제목을 밀어낸다

- **현황**: 유료 + `closed` + `refundStatus: "100"`에서 버튼 대신 표시되는 안내 문구가 헤더
  우측 칸에 들어가 제목이 여러 줄로 밀린다(2026-09-20 실화면).
- **영향**: 드문 상태에서의 외관 문제.
- **처리**: 등재만.
- **격상 트리거**: 대시보드 헤더 수정 시.
- **출처**: 2026-09-20 세션.
...
- **현황**: `src/lib/notifications/sms-templates.ts:8-9` 주석이 모든 템플릿을 90바이트
  이하로 유지하도록 규정하나, `render_delayed` 템플릿이 이를 넘는다. UTF-8 실측
  고정부 97바이트(`{title}` 제외). `{title}` 치환 시 더 늘어난다.
- **이번 커밋과 무관**: 원본 문구 시절에도 고정부 88바이트로 `{title}`이 한 글자만
  들어가면 초과였다. 8/17 실측 과정에서 발견된 기존 상태다. `bf93b72`는 직전 초안
  108바이트를 97바이트로 줄인 것이며 90바이트 달성에는 이르지 못했다.
- **미실측**: 다른 템플릿의 바이트 수는 실측하지 않았다. 같은 파일의 나머지 템플릿도
  초과 상태일 수 있으나 확인한 바 없다.
- **영향**: SOLAPI가 SMS→LMS로 자동 승급하므로 발송 자체는 된다(`:9` 주석). 코드
  강제 검증 로직은 없다. 비용 차이만 발생한다.
- **처리**: 90바이트를 실제로 맞추려면 "문의: 카카오톡 @congre" 제거 수준의 문안
  재설계가 필요해 범위 밖으로 두었다.
...
- **현황**: `src/app/api/render/start/route.ts`의 update 블록이 `refundStatus: "none"`을
  무조건 쓴다. 이미 환불이 확정된 이벤트를 재렌더하면 이 상태가 초기화된다.
- **미확인**: `refundStatus`를 읽는 코드를 확인한 바 없다. 완료 메일의 환불 블록 조건에
  쓰인다는 정황이 CHANGELOG 2026-05-04 항목에 있으나 실제 코드로 대조하지 않았다.
- **처리**: 2026-08-14 `6e18ddd` 작업 시 최소 변경 원칙으로 남겨두었다. 읽는 곳을
  확인하지 않은 채 지우면 완료 메일이 조용히 깨질 수 있다.
- **출처**: 2026-08-14 세션.
- **2026-08-16 재평가**: 재렌더 유료화(`a1735a0`) 작업 중 `render/start`의 리셋 4건을
  검토했다. `participantNotifiedAt` 리셋만 제거하고 `refundStatus`를 포함한 인접 3개는 렌더
  진행 알림 축이라 유지했다. 읽는 곳은 여전히 미확인이다.
- **격상 트리거**: 환불 흐름을 다시 손대게 될 때 / 실제 환불 발생 시.
- **2026-09-20 갱신**: 읽는 곳 확인. `render/start`의 `REFUND_LOCKED` 가드가 덮어쓰기
  (update 블록) 전에 읽고, `check-rendering/route.ts:122`가 읽어 완료 알림의 환불 문구에
  쓴다. 50% 환불 이벤트가 렌더 실패 후 재시작하면 `"none"`으로 덮여 완료 알림에서 환불
  문구가 빠진다.
...
- **2026-08-16 재렌더 축 추가**: 재렌더 결제도 같은 `prepare`를 거치므로 반복 호출 시
  pending이 동일하게 쌓인다. `prepare`가 `payments`를 조회하지 않아 두 번 결제하면 두 번
  청구된다. 구조는 같으나 재렌더는 금액이 실제로 부과되는 축이라 성격이 다르다.
- **격상 트리거**: 실고객 유입 후 중복 청구 문의 발생 시.
...
## deadlineAt이 closedAt보다 뒤에 찍힘 — 관계 미확인 (메모)

- **현황**: 2026-08-15 테스트 결제 실측에서 `closedAt` 16:46:30, `deadlineAt` 16:46:34로
  4초 차이가 확인됐다. 결제 확정이 먼저, 마감 시각이 뒤다.
- **미확인**: 두 필드의 관계를 정의한 코드는 확인하지 않았다. 정상 사양인지 처리 순서에
  따른 부산물인지 미판정.
- **영향**: 환불 기산점은 `closedAt`이므로 환불 계산에는 영향 없음. 다른 로직이
  `deadlineAt`을 읽는지는 미확인.
- **격상 트리거**: `deadlineAt`을 읽는 로직을 손댈 때.
...
- **현황**: `isKeyInEvent`(`src/lib/s3-server.ts:6-9`)는 `startsWith` 비교라
  `events/{eventId}/../다른키` 형태의 문자열이 검사를 통과한다.
- **미실측**: S3는 키를 불투명한 문자열로 다루고 경로 정규화를 하지 않는 것으로 알려져 있어,
  실제로는 존재하지 않는 키가 될 뿐 폴더 밖 접근으로 이어지지 않을 가능성이 높다. 다만 이를
  실제로 확인한 바 없다.
- **처리**: 갈래 A 범위 밖이라 등재만.
- **격상 트리거**: S3 키 정규화 동작을 실측하게 될 때 / 다른 사유로 이 헬퍼를 손대게 될 때.

## presign 발급 성공 로그가 S3 키를 평문 기록

- **현황**: `src/app/api/upload/presign/route.ts:150`이 발급 성공 시 S3 키 전체를 평문 출력한다.
  ```
  console.log(`[presign] kind=${kind} key=${key} contentType=${contentType} expires=300s`);
  ```
- **등급 재평가**: `console.log`라 Vercel 서버 로그에만 남고 클라이언트에 노출되지 않는다.
  악용하려면 로그 접근 권한이 선행 필요하다. 서명된 URL 자체가 유출되던 M-4(`01b80f3` 처리)와는
  계열이 다르다 — M-4는 로그 한 줄이 곧 24시간짜리 다운로드 링크였으나, 이쪽은 키 문자열뿐이다.
- **M-1·M-2 해소로 체인이 끊김**: 2026-08-07 `9779c13`으로 키 프리픽스 소속 검증이 들어가면서
  "키를 알면 남의 이벤트 것을 볼 수 있다"는 연결이 닫혔다. 이 로그가 키를 흘려도 그 키를 자기
  이벤트 문서에 심을 수 없다.
- **처리**: 미처리. 우선순위 낮음.
...
## 인트로/아웃트로 길이 측정 헬퍼에 타임아웃 없음 — 무응답 시 업로드 무반응

- **현황**: `src/app/dashboard/events/[eventId]/page.tsx`의 `measureVideoDuration`은
  `loadedmetadata`·`error` 어느 이벤트도 발생하지 않는 파일을 만나면 Promise가 영원히
  resolve되지 않는다.
- **증상**: 이 검사는 `setIntroUploading(true)`(아웃트로는 `setOutroUploading(true)`) 이전에
  실행되므로 진행 표시가 없다. 호스트에게는 "파일을 골랐는데 아무 반응이 없다"로 보인다.
- **미실현**: 이 조건을 실제로 유발하는 파일은 확인된 바 없다. 비표준 코덱 영상도 통상
  `error` 이벤트는 발생한다(known-issues의 곰믹스 MPEG-4 Part 2 항목 참조).
- **처리 후보**: `Promise.race`로 타임아웃(수 초) 부여 후 NaN resolve.
- **처리**: 현 커밋 범위를 늘리지 않기 위해 등재만 함.
- **격상 트리거**: 호스트가 인트로/아웃트로 파일 선택 후 무반응을 신고할 때.

## 재생 시간 측정 로직 중복 2곳

- **현황**: `src/app/dashboard/events/[eventId]/page.tsx`와 `src/app/upload/[eventId]/page.tsx`가
  각자 재생 시간 측정 구현을 가진다.
- **처리**: 이번 사이클에서 게스트 업로드 경로 무접촉을 우선해 **의도적으로** 중복을 택했다.
- **격상 트리거**: 길이 규칙이 다시 바뀔 때 / 게스트 업로드 경로를 여는 다른 사이클에 착수할 때.

## 안내(maxClipSeconds)와 강제(120초)가 서로 다른 값 — 구조적 갭

- **현황**: 게스트 화면 안내는 이벤트별 `maxClipSeconds`(무료 10 / 유료 10~100 /
  쿠폰 30)를 표시하지만, 실제로 업로드를 막는 검증은 클라이언트
  (`upload/[eventId]/page.tsx:225-230`)·서버(`api/clips/route.ts:24`) 모두
  **120초 하드코드**이며 `maxClipSeconds`를 읽지 않는다. `maxClipSeconds`가
  실제로 반영되는 유일한 지점은 렌더의
  `Math.min(duration, maxClipSeconds)`(`api/render/start/route.ts:100`).
- **결과**: 무료 이벤트에서 60초를 촬영해도 업로드는 통과하고, S3에는 60초 원본이
  저장되며, 완성본에는 앞 10초만 나온다.
- **회피 불가**: iOS·Android 모두 네이티브 카메라 앱으로 촬영하므로 촬영 중 길이를
  강제할 수단이 없다. duration은 파일이 돌아온 뒤에야 알 수 있고, 그 시점의 거부는
  "다시 찍으세요"뿐이라 게스트 이탈 지점이 된다. 따라서 강제를 `maxClipSeconds`로
  조이는 처방은 채택하지 않는다.
- **처리(2026-08-07, `0356ab4`)**: 기대 불일치만 해소. 촬영 직전 안내를
  "앞부분 N초만 영상에 담겨요 (뒤는 잘려요)"로 교체해 게스트가 소감을 앞에
  배치하도록 유도. 강제 로직·나머지 안내 3곳 무접촉.
- **잔존**: S3에 최대 120초 원본이 저장되는 것은 그대로(클립 24h cleanup 대상이라
  누적은 없음). 호스트에게 "게스트 영상이 잘렸다"를 알리는 경로도 없음.
- **격상 트리거**: 호스트가 "게스트 영상이 잘렸는지 몰랐다"를 신고할 때 /
  네이티브 카메라 외 촬영 경로(웹 MediaRecorder 등)를 다시 도입할 때.
- **출처**: 2026-08-07 scout 2회 정찰(값 결정→전달→강제→렌더 4단 체인 전수).

## handoff 3건의 `trim: 0` 기재는 stale — 문서 수정 대신 메모

- **현황**: `docs/handoff/` 3개 파일이 참가자 클립에 `trim: 0`이 있는 것처럼 기재한다.
  실제로는 2026-05-18 `dfe322c`에서 제거됐다 — Shotstack이 clip 레벨 `trim`을
  400(Unknown property)으로 거부했고, 공식 스키마상 `trim`은 asset 레벨 전용이다.
  iOS·Android 필드 테스트 양쪽 실패로 발견된 건.
- **처리**: handoff는 그 시점의 인식을 담은 이력 스냅샷이므로 수정하지 않는다.
  현행 사양 문서인 `decisions/rendering.md`만 2026-08-07에 정정했다.
- **주의**: `trim` 관련 grep 시 handoff 결과가 잡히면 stale로 간주할 것.
  현재 코드에 `trim` 속성은 참가자 클립·인트로/아웃트로 어디에도 없다.

## 문서 규약 메모 — docs/legal/CHANGELOG.md는 최신이 아래

- **현황**: `docs/legal/CHANGELOG.md`는 다른 문서와 달리 최신이 아래다 —
  `[0.1]` → `[0.2]` → `[1.0]` 오름차순. 예정 블록이 맨 아래에 있던 기존 구조를
  유지한 결과다(2026-08-11 확인).
- **주의**: 다른 문서(CHANGELOG.md 등)의 "최신이 위" 규약과 혼동 주의.
...
- 확인법: 이미 사용된 번호를 쿠폰 칸에 재입력 → 얼럿이 "이미 사용된 번호예요…"로 뜨는지. 범용 "이벤트 생성 중 오류가 발생했습니다"가 뜨면 키 통일이 어딘가에서 안 물린 것.
- 처리 시점: 다음 쿠폰 발급·테스트 시 자연 확인.
...
- 확인법: Admin SDK 읽기전용으로 전 계정 emailVerified+가입일 조회 → 미인증 계정 목록화.
- 격상 트리거: 웨딩 라이브 준비 / 실고객 유입 직전. "남의 갇힌 이벤트 복구 경로 부재" known-issue와 한 스크립트로 묶기.
- 현재: FGT 단계라 대상 대부분 테스트/지인 계정. YAGNI 보류.
- 출처: 2026-07-03 이메일 인증 도메인 사고 세션.
...
- **현황**: 결제 연동 시 금액을 서버에서 계산해야 하나 현재는 클라이언트에만 산출
  로직이 있다. 클라이언트 금액을 신뢰하면 2026-08-06 보안 감사 H-2
  (`unlocked:true` 자기부여)와 같은 계열의 결제 게이트 우회가 된다.
- **격상 트리거**: 결제 연동 트랙(ⓐ) 착수 시 서버 금액 산출 구현.
- **출처**: 2026-08-10 세션, 등재는 2026-08-11.

## 과금 모델 미구현 — 마감 시점·실제 개수 정산 사양이 코드에 없음

- **현황**: 과금 모델(마감 시점, 실제 개수 정산) 사양이 코드에 없다. 현 `calcPrice`는
  천장 표시용이다.
...
- 미확인: closed 상태에서 제외 토글이 실제로 노출되는지 확인하지 않았다.
- 처리: 차단하지 않는다(UI 분기 증가 회피, YAGNI). 등재만.
- 격상 트리거: 호스트가 차액 환불을 요구할 때.

## Clip 타입 정의와 실제 저장 필드 불일치
- 현황: src/lib/events.ts:35-44 Clip 인터페이스에 duration이 없으나 api/clips/route.ts:80-88이 실제로 저장한다. sessionToken은 타입에 있으나 POST에서 저장하는 코드를 확인하지 못했다. 대시보드 로컬 타입 ApiClip(dashboard/events/[eventId]/page.tsx:64-73)에도 duration이 없다.
- 영향: 현재 기능 영향 없음. duration은 렌더 시점에 별도로 읽힌다.
- 주의: duration은 결제 금액 계산에 사용하지 않는다(금액의 길이 항은 event.maxClipSeconds 설정값). 타입에 duration이 있다는 이유로 금액 계산에 끌어쓰지 말 것.
- 처리: 타입 정비 사이클에 일괄. 등재만.
...
- **2026-08-11 갱신**: 격상 트리거 (a)가 충족됐다. rayne.co.kr Google Workspace 구축으로 `cs@rayne.co.kr` 수신이 가능해졌고, `hello@congre.kr`은 이제 앱·랜딩 어디에도 없다. 단 코드 상수 `LEAD_TO = "rhcho@naver.com"`은 미변경 — 다음 코드 세션 큐잉 대상.
...
### L8. 랜딩 푸터 = git 외부 직접 수정분 4건 (CD zip 덮어쓰기 주의)
...
### L11. 히어로 완성본 demo.mp4 = 4:3 구워진 띠 + 가로/세로 불일치 (2026-06-27)

- **현황**: 히어로 완성본 샘플 `deploy/videos/demo.mp4`가 4:3(1440×1080) 파일인데, 실제 콘텐츠는 1440×808(≈16:9 가로)이고 상하 각 136px 검은 띠가 구워져 있음(ffmpeg cropdetect 300프레임 전수 `crop=1440:808:0:136` 확정).
- **현재 처치**: 히어로를 가로 16:9 폰 목업(`.hero-demo-phone`) + `object-fit:cover`로 구성 → 16:9 박스가 상하 각 135px를 잘라 띠를 사실상 제거(잔여 ~1px). 좌우 잘림도 거의 0(콘텐츠가 16:9).
- **잠재**: 실제 서비스 완성본은 세로 캔버스인데 히어로 샘플(demo.mp4)만 가로라 톤 불일치. 띠 없는/실제 결과물에 맞는 영상으로 교체 후보.
- **격상 트리거**: 히어로 샘플 영상 교체 착수 시.
- **L 번호 부여 메모**: 현 최대 L10(OG 프록시) 다음 전역 일련번호(CLAUDE.md 학습 룰 #2)에 따라 L11 부여.

### L12. 랜딩 히어로 죽은 코드 (고아 모달 + 후행값 승 잔재) (2026-06-27)

- **현황**: (a) `#demoModal`/`#demoVideo` + JS(`cta === 'demo'`)가 "1분 데모 보기" 버튼 삭제로 완전 고아 — 여는 입구가 없어 도달 불가(JS 에러는 없음). (b) 한 줄 교체를 "추가"로 처리해 남았던 죽은 CSS 줄 패턴(`.hero-demo-video` aspect-ratio 중복, `.howto` max-height/overflow 중복) — 후행값 승이라 동작은 정상이었고 이후 히어로 가로 전환 사이클에서 대부분 정리됨.
- **잠재**: 도달 불가 모달·JS가 파일에 잔존(용량·가독성). 동작 영향 0.
- **격상 트리거**: 랜딩 코드 정리 사이클 착수 시 일괄 제거.
- **L 번호 부여 메모**: L11 다음 전역 일련번호에 따라 L12 부여.

### L13. 음소거 토글 접근성 — aria-hidden 안의 인터랙티브 버튼 (2026-06-27)

- **현황**: `.hero-mute-toggle`이 `aria-hidden="true"`인 `.hero-visual` 안에 있어 키보드 탭·스크린리더로 접근 불가(마우스·터치는 정상). 데모 영상이 장식(aria-hidden) 취급이라 그 안에 인터랙티브 컨트롤을 둔 모순.
...
## participantNotifiedAt이 null인 done 이벤트는 클립이 영구 잔존한다

- **현황**: cleanup cron의 삭제 대상 D-1·D-2·D-3 어디에도 이 조합이 걸리지 않는다. D-3은 `status`가 `["closed","rendering"]`인 경우만 보므로 `done`을 대상에서 제외한다. 결과적으로 `participantNotifiedAt`이 `null`인 `done` 이벤트의 클립은 삭제 시점이 오지 않는다.
- **미확인**: 어떤 경로에서 이 조합이 발생하는지 확인한 바 없다. 2026-08-16 `a1735a0`으로 `render/start`의 리셋을 제거하기 전에는 재렌더가 이 조합을 만들었으나, 그 외 경로는 미조사다.
- **영향**: 개인정보 처리방침이 선언한 클립 보관 기간과 실제 동작이 어긋난다. 재렌더 기한(48시간)도 이 이벤트에서는 사실상 무제한이 된다.
- **처리**: 등재만. 발생 경로 정찰이 선행돼야 한다.
- **격상 트리거**: 실고객 유입 시 / 처리방침 재점검 시.
- **출처**: 2026-08-16 세션.

## 48시간 리터럴 2곳이 서로 다른 축인데 숫자가 같다

- **현황**: `48 * 60 * 60 * 1000`이 두 곳에 하드코딩돼 있다.
  - `src/app/api/cron/cleanup/route.ts:15` — 클립 보관 축
  - `src/app/api/payment/confirm/route.ts:130` — 100% 환불 경계 축
- **위험**: 두 값이 우연히 같을 뿐 서로 다른 정책이다. 한쪽 정책만 바꾸려다 다른 쪽까지 바꾸거나, 한쪽만 고쳐 조용히 어긋날 수 있다. 공유 상수 모듈은 없다.
- **처리**: 등재만. 상수 분리는 값을 실제로 바꿔야 할 때 함께 한다(YAGNI).
- **격상 트리거**: 둘 중 한 축의 시간 정책을 변경할 때.
- **출처**: 2026-08-16 세션.

## 재편집 기능의 용어가 3곳에서 서로 다르다

- **현황**: 같은 기능을 약관은 "재렌더", 랜딩은 "재편집", 서비스 UI는 "다시 만들기"로 부른다.
- **영향**: 약관 조항과 화면을 대조하려는 고객이 같은 기능인지 판단하기 어렵다. 문의 응대 시에도 용어가 흔들린다.
- **처리**: 등재만. 통일하려면 약관 개정이 동반되므로 비용이 작지 않다.
- **격상 트리거**: 약관을 다른 사유로 개정하게 될 때 함께 처리 / 용어 관련 고객 문의 발생 시.
- **출처**: 2026-08-16 세션.

## 재렌더 결제가 서버에서 강제되지 않는다

- **현황**: `src/app/api/render/start/route.ts`의 결제 가드는 `plan === "paid" && !unlocked` 조건이다. 재렌더 시점에는 최초 결제로 `unlocked`가 이미 `true`이므로 이 가드를 통과한다. `/api/render/start`를 직접 호출하면 재렌더 결제 없이 렌더가 실행된다.
- **손실 규모**: 호스트가 우회할 수 있는 대상은 자기 이벤트뿐이고, 손실은 렌더 1회 원가에 한정된다. 타인의 이벤트를 렌더하거나 금액을 조작하는 경로는 아니다.
- **처리**: 등재만. 막으려면 재렌더 티켓 등 별도 장치가 필요하다.
- **격상 트리거**: 실제 우회 사례 관측 시 / 재렌더 1회 원가가 유의미해질 때.
- **출처**: 2026-08-16 세션.
- **2026-09-20 갱신**: `fc1e487`로 대시보드 재시작 경로는 정리됐으나 이 구멍은 그대로다.
  같은 커밋은 `render/start`에 `REFUND_LOCKED` 가드(유료 + closed + 전액 환불 확정)만
  추가했다. 서버 강제는 다음 사이클로 보류(decisions/market-product.md 2026-09-20).

## 재렌더 모달이 구체 금액을 표시하지 못한다

- **현황**: `host/events/[eventId]` GET 응답에 `firstPaidAmount`가 없어, 재렌더 모달은 "처음 결제 금액의 80%"라고만 안내한다. 실제 금액은 결제 페이지에 도달해야 처음 보인다.
- **약관 관점**: 제14조 ③의 사전 고지·확인 의무는 결제 페이지 고지로 충족된다. 위반은 아니다.
- **처리**: 등재만. 해소하려면 GET 응답에 필드 추가가 필요하다.
- **격상 트리거**: 금액을 모른 채 결제 페이지까지 갔다가 이탈하는 패턴이 관측될 때.
- **출처**: 2026-08-16 세션.

## 사업자 정보 블록이 두 페이지에 독립 하드코딩돼 있다

- **현황**: 사업자 정보(상호·대표자·사업자등록번호·주소 등)가 `src/app/terms/page.tsx`와 `src/app/privacy/page.tsx`에 각각 따로 하드코딩돼 있다. 공유 상수나 컴포넌트가 없다.
- **위험**: 정보가 바뀔 때 한쪽만 고치면 두 법적 문서가 서로 다른 사업자 정보를 표시한다. 통신판매업 신고번호가 발급되면 실제로 두 곳을 함께 고쳐야 한다.
- **처리**: 등재만.
- **격상 트리거**: 통신판매업 신고번호 발급 시 / 사업자 정보 변경 시.
- **출처**: 2026-08-16 세션.
- **2026-09-16 트리거 발동**: 통신판매업 신고번호 발급으로 두 곳을 함께 교체했다. 공유 상수·컴포넌트 추출은 하지 않고 직접 교체로 처리했다(YAGNI — 사업자 정보 변경 빈도가 낮다). 랜딩(git 밖)에도 같은 정보 블록이 4곳 있어 전체 6곳·5개 파일이다. 남은 트리거: 070 유선번호 신청 시.

## 국민카드 위젯 표시가 legal 기록과 어긋난다 (2026-08-24)

`decisions/legal.md` 2026-08-16 결정 4는 "토스 승인이 나도
국민카드는 열리지 않는다"고 기록한다. 그러나 2026-08-24 결제
페이지 실측에서 토스 결제위젯 카드사 목록에 국민이 표시됐다.

위젯 표시와 실제 승인 가능 여부는 별개일 수 있어 현 시점에서
어느 쪽이 맞는지 판정하지 않는다. 심사 완료 후 실결제로 확인할 것.
