...
## render_delayed 계열 "30분" 문구가 실동작과 불일치

- **현황**: 지연 알림 3곳이 "30분"을 하드코딩하고 있다.
  - `src/emails/render-delayed.ts:27` — "30분 이내에도 완료되지 않을 경우 결제 금액의
    50%가 자동으로 환불 확정"
  - `src/lib/notifications/scenarios/render-delayed.ts:26` — plain text 판
  - `src/lib/notifications/sms-templates.ts:15-16` — SMS 판
- **어긋남의 크기**: 2026-08-14 `6e18ddd` 이전에도 이미 어긋나 있었다. 구 공식은
  `renderEstimateMin + 30분`(클립 수에 따라 가변, 최소 45분)이라 문구의 "30분"과 달랐다.
  `6e18ddd`로 기준이 결제 후 4시간이 되면서 차이가 15분에서 3시간 30분으로 커졌다.
- **영향**: 고객에게 직접 발송되는 문구다. "30분 후 50% 환불 확정"을 받은 호스트가
  3시간 30분을 더 기다리게 된다.
- **처리**: 이번 커밋 범위 밖. 등재만.
- **격상 트리거**: 유료 결제 실사용 개시 시점 — 그 전에 반드시 처리.
- **출처**: 2026-08-14 scout 정찰(환불 구간 실측, 요청 범위 밖 발췌분).
