...
## ⑦ Shotstack→S3 복사 막힘으로 미검증인 구현 완료 기능

> ⑦(렌더 결과물의 운영자 S3 복사가 Access Denied로 실패 → cron HeadObject 실패 → 이벤트 rendering 고착, done 미도달) 때문에 코드·커밋·빌드·린트는 통과했으나 실제 영상 결과물/완료 후 흐름으로는 검증 못 한 기능 목록. ⑦ 풀리면 이 목록 순서로 검증. 검증 완료 항목은 known-issues-resolved.md로 이동.

### A. 영상 출력물을 봐야 검증되는 기능 (내용 레벨)

⑦은 "Shotstack→운영자 S3 복사"를 막지 렌더 자체는 완료됨. 따라서 A 그룹은 **Shotstack 대시보드 렌더 결과 미리보기로 ⑦ 무관하게 지금 검증 가능할 수도 있음**(미확정 — 6/04 destinations 도입이 출력 경로를 바꿨다면 미리보기 불가일 수 있음. 대시보드에서 박힌 렌더 1건 재생 여부부터 확인).

1. BGM 분위기 선택 (calm/lively/epic, S3 곡 풀 랜덤 픽) — UI·저장 검증됨. 분위기별 곡 실제 적용 미검증. 함정: 현 lively 곡이 폴백 bgm.mp3와 동일 곡 → calm/epic 다른 곡으로 렌더해야 소리로 구분.
2. 한글 인트로/아웃트로 (Shotstack rich-text + NotoSansKR TTF) — 한글 깨짐 없이 렌더되는지.
3. outroText + outroMedia 동시 입력 직렬 배치 ([A] 분기) — 둘 다 넣었을 때 순서대로 붙는지.
4. 참가자 클립 음량 페이드 (volumeEffect: fadeInFadeOut) + BGM 믹싱.
5. 트랜지션 in/out 분리 (pickSequence 2회 호출) — 전환 다양성.
6. 한글 자막 — 영상에 제대로 박히는지.
7. 여러 클립 → 하나의 회고 영상 합성 (서비스 핵심) — 위 요소 합쳐진 최종 결과물 전체.

### B. 렌더 완료 후 앱 흐름 (⑦에 직접 막힘 — 대시보드로도 검증 불가)

1. 완성본 저장 + done 상태 전환 (videoUrl, cron check-rendering의 HeadObject) — ⑦ 핵심 = ② 앱 고착(rendering 박힌 이벤트 6개+).
2. 렌더 완료 알림 (Resend 이메일 + SOLAPI SMS) — done 도달해야 트리거.
3. 참가자 결과 알림 — 완성본 나와야 전달.
4. 재렌더 흐름 (done 상태 노출 + 클립 토글 모달).
5. 완성본 7일 보관 + 자동 삭제 cron (videoDeletedAt). (클립 24h 삭제는 별개 동작)
6. 완성본 SNS 공유 (카카오·링크 복사) — 버튼 존재. "완성본 URL 대상" 공유는 미검증.

참고: 렌더 지연(10분 초과)·렌더 실패 알림은 stuck 때문에 의도치 않게 발현됐을 수 있음(박힌 이벤트가 10분 초과 시). 실측 미확인 — 추정.

### 분류 미상 (시야 밖)
- live lottery 기능 (커밋 898b4d1) — 채팅 클로드 컨텍스트에 동작·렌더 의존 여부 정보 없음. 렌더 의존이면 A/B에 편입 필요. 운영자/CC가 앎.
- PROJECT.md 미반영 최근 기능이 있으면 본 목록에서 누락됐을 수 있음.

### 출처
2026-06-07 v2 채팅 세션. PROJECT.md 완료 기능 + 2026-06-07 BGM 핸드오프 기준 추림.
