...
## 회원가입 화면 "무료로 테스트해 볼 수 있어요" 문구 — 카드사 심사 맥락 충돌

- **현황**: `/signup` 화면 상단 안내 문구가 "가입하면 바로 무료로 테스트해 볼 수 있어요"다.
  실제 의미는 무료 체험 안내이나, 토스 1차 심사 지적 사항이 "테스트 상품 불가"였으므로
  심사 서류에 이 단어가 노출되면 오해 소지가 있다.
- **영향**: 제출 캡처 ④(로그인·회원가입)에 그대로 찍힌다.
- **해소 방법**: "가입하면 바로 무료로 시작할 수 있어요" 등으로 교체 후 배포 →
  캡처 ④ 재촬영.
- **격상 트리거**: 2차 회신 제출 전. 문자열 1건이라 분량은 작다.
...
- **격상 트리거**: 환불 흐름을 다시 손대게 될 때 / 재렌더 유료화(B5) 착수 시.
- **출처**: 2026-08-14 세션.
