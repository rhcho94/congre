...
## 게스트 클립 길이 안내 문구와 maxClipSeconds 불일치 의심 — 미확인

- **현황**: `src/app/upload/[eventId]/page.tsx`의 길이 초과 안내가 "2분 이내로 촬영해주세요"인데
  `maxClipSeconds` 기본값은 15다.
- **미확인**: 실제로 무엇이 강제되는지 코드 정찰 미실시.
- **처리**: 이번 커밋 범위 밖.
- **격상 트리거**: 게스트가 클립 길이로 혼선을 보고할 때.
